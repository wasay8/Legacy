...
# === Embedding function ===
def create_vector_embedding_with_metadata(counsel_data_path, kaggle_data_path):
...
with st.sidebar:
    ...
    if st.button("🔍 Load & Embed Data"):
        with st.spinner("Processing and embedding... (1-2 minutes)"):
            vectors = create_vector_embedding_with_metadata(counsel_path, kaggle_path)
            st.session_state["vectors"] = vectors
        st.success("✅ Data embedded!")
...
if interaction_mode == "🔍 Search Database":
    ...
    # Query the database (RAG) for response
    if "vectors" not in st.session_state:
        ...
        if user_prompt:
            ...
            # Show sources in a collapsible dropdown (expander)
